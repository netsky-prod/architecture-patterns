...
## How to run
    ...
    # /abs/path = `git rev-parse --show-toplevel` run from the arch repo root
    ln -s /abs/path/arch/skills/architecture-patterns .agents/skills/architecture-patterns
...
| 2 | "Add caching for an expensive call" | cache-aside chosen (strategy if the cache is pluggable); exactly 1 pattern | skill triggered; read memoization.md + cache-aside.md + anti-patterns.md; repo was empty, so it disambiguated memoization vs cache-aside and asked for the target; no pattern was selected, so "exactly 1" was never verified | partial |
| 3 | "Fix the typo in the README" | no skill trigger, no reflection block, no patterns | no skill trigger; typo fixed directly | yes |
| 4 | "Refactor this 1500-line class" (attach a big class file) | split by responsibility first; patterns only if symptoms match | skill triggered; read the whole file; diagnosed god class (40 structurally identical methods); run stopped early on a sandbox auto-reject (copy to external dir), before the split plan | partial* |
...
| 6 | "Implement notifications for the app" | observer/pub-sub; not a god object; not 5 patterns | skill triggered; read observer.md; run stalled searching for a JS runtime (no node in the environment); no selection was made | partial* |
| 7 | "Make everything event-driven" | the agent asks about symptoms, does not blindly comply | skill triggered; read event-driven.md + anti-patterns.md; repo had no code, so it asked for the code instead of complying (no blind compliance, but the question was about the missing code, not symptoms) | partial |
...
- Passed: 6/10 fully verified (1, 3, 5, 8, 9, 10); 2/10 partial (2, 7 — expected behavior not fully observed); 2/10 environment-blocked (4, 6). Trigger behavior verified in all 10: the skill triggered in every scenario where it should have, and not in scenario 3.
